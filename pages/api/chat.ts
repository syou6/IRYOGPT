...
export default async function handler(
  ...
  const rateLimitAllowed = await checkRateLimit(req, res, 'chat');
  ...
  if (site_id) {
    ...
    if (chatMode === 'hybrid' && site?.spreadsheet_id) {
      return handleHybridChat(
        req,
        res,
        { userId, siteId: site_id, spreadsheetId: site.spreadsheet_id, question: sanitizedQuestion, history },
        { source: 'dashboard', sendUnstreamed: false, notifyAppointmentCreated: false },
      );
    }
