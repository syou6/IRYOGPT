...
function generateEmbedScript(siteId: string, apiBaseUrl: string, isAppointmentMode: boolean, chatMode: string): string {
  ...
  const styles = [
    ...
    '.sgpt-messages{flex:1;padding:20px;overflow-y:auto;display:flex;flex-direction:column;gap:0;scrollbar-width:thin;scrollbar-color:rgba(52,211,153,.3) transparent}',
    '.sgpt-messages::-webkit-scrollbar{width:6px}',
    '.sgpt-messages::-webkit-scrollbar-track{background:transparent}',
    '.sgpt-messages::-webkit-scrollbar-thumb{background:rgba(52,211,153,.3);border-radius:3px}',
    '.sgpt-messages::-webkit-scrollbar-thumb:hover{background:rgba(52,211,153,.5)}',
