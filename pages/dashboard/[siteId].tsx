...
export default function SiteChat() {
  ...
  const markdownComponents = useMemo<MarkdownComponents>(() => ({
    a({ children, ...props }) {
      return (
        <a
          ...
          className="text-emerald-600 underline decoration-emerald-500/50 underline-offset-2 transition hover:text-emerald-800"
        >
          {children}
        </a>
      );
    ...
    code({ inline, className, children, ...props }) {
      if (inline) {
        return (
          <code
            {...props}
            className={`rounded-md bg-premium-elevated px-1.5 py-0.5 text-[0.85em] text-emerald-700 ${className || ''}`.trim()}
          >
            {children}
          </code>
        );
      ...
      return (
        <pre className="mt-3 overflow-x-auto rounded-2xl border border-premium-stroke bg-slate-100 p-3">
          <code {...props} className={`text-[0.9em] leading-relaxed text-emerald-800 ${className || ''}`.trim()}>
            {children}
          </code>
        </pre>
      );
    ...
    ul({ children, ...props }) {
      return (
        <ul {...props} className="ml-4 list-disc space-y-1 text-premium-text">
          {children}
        </ul>
      );
    },
    ol({ children, ...props }) {
      return (
        <ol {...props} className="ml-4 list-decimal space-y-1 text-premium-text">
          {children}
        </ol>
      );
    },
    p({ children, ...props }) {
      return (
        <p {...props} className="mb-3 text-premium-text last:mb-0">
          {children}
        </p>
      );
    },
  ...
  return (
    <Layout>
      ...
      <div className="relative mx-auto max-w-6xl px-4 py-6 text-premium-text lg:py-8">
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row">
          {/* メインコンテンツ（チャット） */}
          <div className="flex min-h-[70vh] flex-1 flex-col overflow-hidden rounded-[32px] border border-premium-stroke bg-premium-surface shadow-[0_25px_60px_rgba(0,0,0,0.08)] backdrop-blur-2xl">
            {/* ヘッダー */}
            <div className="flex flex-col gap-3 border-b border-premium-stroke px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
              <div className="min-w-0">
                <Link
                  href="/dashboard"
                  className="mb-1 inline-flex items-center text-[11px] uppercase tracking-[0.35em] text-emerald-600/80"
                >
                  ← ダッシュボード
                </Link>
                <h1 className="truncate text-2xl font-semibold text-premium-text">{site.name}</h1>
                <p className="mt-1 break-all text-xs text-premium-muted">{site.base_url}</p>
              </div>
              <div className="flex flex-col gap-2 sm:items-end">
                {/* チャットモード切替 */}
                <div className="flex items-center gap-2">
                  <span className="text-xs text-premium-muted">モード:</span>
                  <div className="flex rounded-full border border-premium-stroke bg-premium-surface p-0.5">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <span className={`rounded-full border px-3 py-1 text-xs font-medium ${
                    site.status === 'ready'
                      ? 'border-emerald-400/40 bg-emerald-400/15 text-emerald-800'
                      : 'border-premium-stroke bg-premium-elevated text-premium-text'
                  }`}>
                  ...
                  {embedHref && (
                    <Link
                      href={embedHref}
                      className="rounded-full border border-premium-stroke bg-premium-surface px-4 py-1.5 text-xs font-medium text-premium-text transition hover:bg-premium-elevated"
                    >
                      埋め込み設定
                    </Link>
                  )}
                  {siteIdForLinks && (
                    <Button
                      ...
                      className="rounded-full border border-emerald-400/40 bg-emerald-400/15 px-4 py-1.5 text-xs font-medium text-emerald-700 transition hover:bg-emerald-400/25"
                    >
                      質問インサイト
                    </Button>
                  )}
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-full border border-premium-stroke bg-premium-surface px-4 py-1.5 text-xs font-medium text-premium-text transition hover:bg-premium-elevated lg:hidden"
                    aria-label="学習履歴を表示"
                  >
                    学習履歴
            ...
            <div className="flex-1 overflow-y-auto px-4 py-6 sm:px-6">
              <div className="mx-auto max-w-3xl">
                {chatMessages.length === 0 ? (
                  <div className="mt-8 text-center text-premium-muted">メッセージがありません</div>
                ) : (
                  <div className="space-y-4">
                          ...
                          <div
                            className={`max-w-[85%] rounded-3xl px-4 py-3 text-sm leading-relaxed shadow-lg sm:max-w-[80%] ${
                              message.type === 'userMessage'
                                ...
                                : 'border border-premium-stroke bg-premium-elevated text-premium-text'
                            }`}
                          >
                              ...
                              <>
                                ...
                                {bestSource && (
                                  <a
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="mt-4 block rounded-2xl border border-premium-stroke bg-premium-surface px-4 py-3 text-xs text-premium-text transition hover:border-emerald-400/60 hover:bg-emerald-500/5"
                                  >
                                    <p className="text-[11px] font-semibold uppercase tracking-[0.3em] text-emerald-600/70">
                                      引用元
                                    </p>
                                    {bestSource.title && (
                                      <p className="mt-2 text-base font-semibold text-premium-text">
                                        {bestSource.title}
                                      </p>
                                    )}
                                    <p className="mt-1 break-words text-[11px] text-emerald-600">
                                      {bestSource.url}
                                    </p>
                    ...
                    {loading && !pending && (
                      <div className="flex justify-start">
                        <div className="rounded-3xl border border-premium-stroke bg-premium-elevated px-4 py-2">
            ...
            {isAppointmentMode && site.spreadsheet_id && site.status === 'ready' && (
              <div className="border-t border-premium-stroke px-4 pt-4 sm:px-6">
                <div className="mx-auto max-w-3xl">
                  <div className="mb-2 text-xs text-premium-muted">よくある質問</div>
                  <div className="flex flex-wrap gap-2">
                    {APPOINTMENT_QUICK_REPLIES.map((reply, index) => (
                      <button
                        ...
                        disabled={loading}
                        className="rounded-full border border-emerald-400/30 bg-emerald-400/10 px-4 py-2 text-sm font-medium text-emerald-700 transition hover:border-emerald-400/50 hover:bg-emerald-400/20 disabled:cursor-not-allowed disabled:opacity-50"
                      >
                        {reply.label}
                      </button>
                    ))}
            ...
            {/* 入力フォーム */}
            <div className="border-t border-premium-stroke px-4 py-5 sm:px-6">
              <div className="mx-auto max-w-3xl">
                <ChatInput
          ...
          >
            <div className="flex h-full flex-col rounded-[28px] border border-premium-stroke bg-premium-surface p-5 shadow-[0_25px_60px_rgba(0,0,0,0.08)] backdrop-blur-2xl">
              <div className="mb-4 flex items-center justify-between">
                <h2 className="text-lg font-semibold text-premium-text">学習履歴</h2>
                <button
                  onClick={() => setShowSidebar(false)}
                  className="rounded-full border border-premium-stroke p-1 text-premium-muted hover:text-premium-text lg:hidden"
                  aria-label="閉じる"
                >
              ...
              <div className="flex-1 overflow-y-auto">
                {trainingJobs.length === 0 ? (
                  <p className="text-sm text-premium-muted">学習履歴がありません</p>
                ) : (
                  <div className="space-y-3">
                    {trainingJobs.map((job) => (
                      <div
                        key={job.id}
                        className="rounded-2xl border border-premium-stroke bg-premium-surface p-3 text-sm text-premium-text"
                      >
                          ...
                          <span
                            className={`rounded-full border px-2.5 py-1 text-xs font-semibold ${
                              job.status === 'completed'
                                ...
                                : job.status === 'running'
                                ? 'border-cyan-400/40 bg-cyan-500/15 text-cyan-700'
                                : 'border-premium-stroke bg-premium-elevated text-premium-text'
                            }`}
                          >
                            {getStatusLabel(job.status)}
                          </span>
                          <span className="truncate text-xs text-premium-muted">{formatDate(job.created_at)}</span>
                        </div>
                        {job.status === 'running' && job.total_pages > 0 && (
                          <div className="mb-2">
                            <div className="mb-1 flex justify-between text-[11px] text-premium-muted">
                              <span>進捗</span>
                              <span>
                                {job.processed_pages} / {job.total_pages}
                              </span>
                            </div>
                            <div className="h-2 w-full rounded-full bg-premium-elevated">
                              <div
                        ...
                        {job.finished_at && (
                          <div className="text-xs text-premium-muted">完了: {formatDate(job.finished_at)}</div>
                        )}
                        {job.metadata?.detection_method && (
                          <div className="mt-2 text-xs">
                            <span className="font-medium text-premium-text">検出方法:</span> {job.metadata.detection_method}
                          </div>
                        )}
                        {job.metadata?.url_count !== undefined && (
                          <div className="mt-1 text-xs text-premium-muted">
                            <span className="font-medium text-premium-text">学習URL数:</span> {job.metadata.url_count}件
                            {job.metadata.url_count === 1 && job.metadata.detection_method?.includes('ベースURLのみ') && (
                              <span className="ml-1 text-orange-600">（ベースURLのみ）</span>
                            )}
                            {job.metadata.url_count > 1 && <span className="ml-1 text-emerald-600">✓</span>}
                          </div>
                        )}
                        {job.metadata?.detected_sitemap_url && (
                          <div className="mt-1 text-xs">
                            <span className="font-medium text-premium-text">サイトマップ:</span>{' '}
                            <a
                              href={job.metadata.detected_sitemap_url}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="break-all text-emerald-600 underline-offset-4 hover:underline"
                            >
                              {job.metadata.detected_sitemap_url}
                            </a>
                          </div>
                        ...
                        {job.metadata?.urls && job.metadata.urls.length > 0 && (
                          <details className="mt-2 rounded-xl border border-premium-stroke bg-premium-surface p-2">
                            ...
                            <div className="mt-2 max-h-40 overflow-y-auto text-[11px]">
                              <ul className="space-y-1 font-mono">
                                {job.metadata.urls.map((url, idx) => (
                                  <li key={idx} className="break-all text-premium-text">
                                    {idx + 1}. {url}
                                  </li>
                                ))}
                              </ul>
                        ...
                        {job.error_message && (
                          <div className="mt-2 text-xs text-rose-600">
                            <span className="font-medium text-rose-700">エラー:</span> {job.error_message}
                          </div>
                        )}
                      </div>
                    ))}
