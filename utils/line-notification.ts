...
/**
 * LINEクライアントを取得（サイトID経由）
 * LINE連携が無効・未設定の場合は null を返す
 */
async function getLineClientForSite(
  siteId: string
): Promise<messagingApi.MessagingApiClient | null> {
...
export async function sendAppointmentNotification(
  ...
  try {
    const client = await getLineClientForSite(siteId);
    if (!client) {
      return false;
    }
